let secondary_front_bg = document.getElementById("secondary-front-bg");
let primary_front_bg = document.getElementById("primary-front-bg");
let front_bg = document.getElementById("front-bg");
let mid_bg = document.getElementById("mid-bg");
let sun = document.getElementById("sun");
let cloudI = document.getElementById("cloudI");
let back_bg = document.getElementById("back-bg");
let leftsky = document.getElementById("left-valve-sky");
let rightsky = document.getElementById("right-valve-sky");
let leftadmin = document.getElementById("left-valve-admin");
let rightadmin = document.getElementById("right-valve-admin");
let leftearth = document.getElementById("left-valve");
let rightearth = document.getElementById("right-valve");
let chain = document.getElementById("chain");
let leftcoin = document.getElementById("left-valve-coin");
let rightcoin = document.getElementById("right-valve-coin");
let leftfeed = document.getElementById("left-valve-feed");
let rightfeed = document.getElementById("right-valve-feed");
let leftpartner = document.getElementById("left-valve-partners");
let rightpartner = document.getElementById("right-valve-partners");
// let space = document.getElementById('op-space');
// let world = document.getElementById('op-world');
// let admin = document.getElementById('op-admin');
// let pepe = document.getElementById('pepe-landing');

window.addEventListener("scroll", function () {
  let value = window.scrollY;
  secondary_front_bg.style.top = value * 0.1 + "px";
  primary_front_bg.style.top = value * 0.2 + "px";
  front_bg.style.top = value * 0.4 + "px";
  mid_bg.style.top = value * 0.6 + "px";
  sun.style.top = value * 0.4 + "px";
  cloudI.style.left = value * 0.5 + "px";
  back_bg.style.top = value * 0.8 + "px";
  leftsky.style.left = value * -0.5 + "px";
  rightsky.style.left = value * 0.5 + "px";
  leftearth.style.left = value * -0.25 + "px";
  rightearth.style.left = value * 0.25 + "px";
  leftadmin.style.left = value * -0.15 + "px";
  rightadmin.style.left = value * 0.15 + "px";
  chain.style.top = value * 0.09 + "px";
  leftcoin.style.left = value * -0.115 + "px";
  rightcoin.style.left = value * 0.115 + "px";
  leftpartner.style.left = value * -0.08 + "px";
  rightpartner.style.left = value * 0.08 + "px";
  leftfeed.style.left = value * -0.05 + "px";
  rightfeed.style.left = value * 0.05 + "px";
  // space.style.left = value * 0.40 + 'px';
  // world.style.top = value * 0.40 + 'px';
  // admin.style.top = value * 0.40 + 'px';
  // pepe.style.top = value * .65 + 'px';
});
